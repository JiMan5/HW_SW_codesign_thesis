...
/* Forward declaration so we can print at exit */
static void report_link_transport_counts(void); //jim
...
void path_transport_field( su3_vector *src, su3_vector *dest, int parity,
  ...
  if( length > 0 ){
	...
	if( j==length-1 ){
	  FORSOMEPARITY_OMP(i,s,tmp_otherparity,){
	        tmp_src[i] = src[i];
	    } END_LOOP_OMP;
	}
	...
	if( GOES_FORWARDS(dir[j]) ) {
	    ...
	    wait_gather(mtag0);
	    FORSOMEPARITY_OMP(i,s,tmp_parity,){
...
//TEST ME
	...
	dir, EVENANDODD, gen_pt[0] );
    wait_gather(mtag0);
    FORALLSITES_OMP(i,s,){ dest[i] = *(su3_matrix *)(gen_pt[0][i]); } END_LOOP_OMP;
    cleanup_gather(mtag0);
