...
static void write_lattice(const char *fname) {
  FILE *f = fopen(fname, "wb");
  if(!f){ perror(fname); return; }

  fwrite(&sites_on_node, sizeof(size_t), 1, f);
  fwrite(lattice, sizeof(site), sites_on_node, f);
  fclose(f);
}
...
//int tempflops = 0; //TEMP
  ...
  if(!ff_first_call_logged) {
    ...
    // Lattice snapshot before
    write_lattice("ff_lattice_before.bin");
...
if(!ff_first_call_logged) {
  ...
  write_lattice("ff_lattice_after.bin");
